...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # positive_sample: a positive triple, LongTensor, [3]
        # negative_sample: some negative entities, LongTensor, [negative_sample_size]
        # subsampling_weight: a subsampling_weight, FloatTensor, [1]
        # self.mode: sample mode, a str
    ...
    def collate_fn(data):
        ...
        subsample_weight = torch.cat([item[2] for item in data], dim=0)
        mode = data[0][3]
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 0 if rand_head / rand_tail are not in triple_set
        # -1 if rand_head / rand_tail are in triple_set
        # specially, 0 if rand_head == head
        if self.mode == 'head-batch':
        ...
        # positive_sample: a positive triple, LongTensor, [3]
        # negative_sample: all entities, LongTensor, [nentity]
        # filter_bias: filter_bias, valued from {0, -1}, FloatTensor, [nentity]
        # self.mode: sample mode, a str
